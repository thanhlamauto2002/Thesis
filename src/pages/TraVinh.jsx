...
function TraVinh({ data1 }) {
  const navigate = useNavigate()

  const [chartData, setChartData] = useState(() => {
    const storedData = localStorage.getItem('traVinhChartData');
    ...
    const updateChartData = () => {
      if (data1 && data1.createdAt && !isNaN(new Date(data1.createdAt))) {
        ...
        setChartData(prevData => {
          ...
          localStorage.setItem('traVinhChartData', JSON.stringify(updatedData));
          setDataUpdated(true);
          return updatedData;
        });
      }
